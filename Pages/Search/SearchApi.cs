...
#pragma warning restore CS4014
            ...
            if (res.status == 200)
            {
                ...
                var listOfNouns = new List<string>();
                var listOfVerbs = new List<string>();
                foreach (var noun in res.nouns) listOfNouns.Add(noun);
                foreach (var verb in res.verbs) listOfVerbs.Add(verb);


                foreach (var noun in listOfNouns.Distinct().ToList())
                {
...
#pragma warning restore CS4014
        ...
        public static async Task<dynamic> GetSearchResults(string searchQuery)
        {
            ...
            var keywordResults = new List<string>();
            foreach (var noun in keywordResultsRaw.nouns) keywordResults.Add(noun);

            foreach (var verb in keywordResultsRaw.verbs) keywordResults.Add(verb);
